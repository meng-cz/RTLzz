...
RTL emitter 已修正此前合并触发的 `stdmix_mul_s8` 问题：每个乘法操作数先独立按 signed view 扩展/截断到结果宽度，随后按无符号位模式相乘并显式截断。避免 SV 混合符号表达式抑制符号扩展，正确性不再依赖位宽优化。

完整 int_misc 在默认优化及 `--beopt none` 下各 100 组随机差分通过；另各 90 组确定性边界组合通过，覆盖 a/b 的 0、1、127、128、255（b 另含 201）与 word 的 0、32768、65535。新增反向混合符号、双有符号、不同位宽及截断输出，S11 emitter 单元测试通过。
